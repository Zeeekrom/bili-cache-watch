...
const translations = {
  'zh-Hans': {
    ...
    add: '添加',
    projectIntroTitle: '项目说明',
    projectIntroText: 'Bili Cache Watch 是一个本地运行的哔哩哔哩缓存管理工具，用来导入缓存视频、检查公开视频状态，并在视频异常时保留本地预览入口。',
    projectBasicTitle: '基本介绍',
    projectBasicText: '项目面向本地缓存整理场景：你可以手动添加 BV 号，也可以从模拟器、本地文件夹或 Windows MTP 手机路径读取 entry.json，集中管理缓存条目。',
    projectTechTitle: '技术介绍',
    projectTechText: '后端基于 Node.js、Express 和 SQLite，定时检查视频状态；导入模块解析缓存元数据，使用 ffmpeg 生成可预览的 MP4，并支持本地弹幕播放与标题翻译缓存。',
    projectUsageTitle: '使用方法',
    projectUsageText: '先运行 npm install 和 npm run dev，打开 localhost:3000；登录后添加 BV 号或导入缓存，再点击全部检查筛选异常视频，必要时使用预览或下载 MP4。',
    cacheImport: '缓存导入',
  ...
  'zh-Hant': {
    ...
    add: '新增',
    projectIntroTitle: '專案說明',
    projectIntroText: 'Bili Cache Watch 是一個在本機執行的嗶哩嗶哩快取管理工具，用來匯入快取影片、檢查公開影片狀態，並在影片異常時保留本機預覽入口。',
    projectBasicTitle: '基本介紹',
    projectBasicText: '專案面向本機快取整理場景：你可以手動新增 BV 號，也可以從模擬器、本機資料夾或 Windows MTP 手機路徑讀取 entry.json，集中管理快取項目。',
    projectTechTitle: '技術介紹',
    projectTechText: '後端基於 Node.js、Express 和 SQLite，定時檢查影片狀態；匯入模組解析快取中繼資料，使用 ffmpeg 產生可預覽的 MP4，並支援本機彈幕播放與標題翻譯快取。',
    projectUsageTitle: '使用方法',
    projectUsageText: '先執行 npm install 和 npm run dev，開啟 localhost:3000；登入後新增 BV 號或匯入快取，再點擊全部檢查篩選異常影片，必要時使用預覽或下載 MP4。',
    cacheImport: '快取匯入',
  ...
  'en-AU': {
    ...
    add: 'Add',
    projectIntroTitle: 'Project Overview',
    projectIntroText: 'Bili Cache Watch is a local Bilibili cache management tool for importing cached videos, checking public video status, and keeping a local preview path when a video becomes abnormal.',
    projectBasicTitle: 'Basic Introduction',
    projectBasicText: 'The project is built for local cache organisation: add BV IDs manually, or read entry.json files from an emulator, a local folder, or a Windows MTP phone path to manage cache items in one place.',
    projectTechTitle: 'Technical Introduction',
    projectTechText: 'The backend uses Node.js, Express, and SQLite with scheduled status checks. Importers parse cache metadata, ffmpeg creates previewable MP4 files, and the UI supports local danmaku playback plus cached title translations.',
    projectUsageTitle: 'How To Use',
    projectUsageText: 'Run npm install and npm run dev, then open localhost:3000. After logging in, add BV IDs or import cache files, run Check All to filter abnormal videos, and use Preview or Download MP4 when needed.',
